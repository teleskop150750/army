...
class ProductModel extends AdminModel
{
    ...
    /**
     * запмсать связанные товары
     * @param int $id
     * @param array $data
     */
    public function editRelatedProduct(int $id, array $data): void
    {
    ...
    public function saveGallery($id): void
    {
     ...
     * получить фильтры
     * @param int $id id товара
     * @return array|null фильтры
     */
    public function getFilter(int $id): ?array
    {
        return R::getCol('SELECT attr_id FROM attribute_product WHERE product_id = ?', [$id]);
    }

    /**
     * получить связанные товары
     * @param int $id id товара
     * @return array|null связанные товары
     */
    public function getRelated(int $id): ?array
    {
        return R::getAll(
            "
                SELECT related_product.related_id, 
                       product.title 
     ...
     * получить галерею
     * @param int $id id товара
     * @return array|null галерея
     */
    public function getGallery(int $id): ?array
    {
        return R::getCol('SELECT img FROM gallery WHERE product_id = ?', [$id]);
    }
    ...
    {
        return R::exec("DELETE FROM gallery WHERE product_id = ? AND img = ?", [$id, $src]);
    }
